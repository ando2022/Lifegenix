...
export default function SignUpPage() {
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (error) {
        await trackError(new Error(error.message), 'signup');
        toast({
        ...
        if (data.user) {
          await trackUserRegistration(data.user, {
  ...
  const handleOAuthSignUp = async (provider: 'google') => {
    ...
    try {
      ...
      if (error) {
        await trackError(new Error(error.message), `signup_${provider}`);
        toast({
          title: "Sign up failed",
          description: error.message,
          variant: "destructive",
        ...
        await trackUserRegistration(null, {
        ...
        <Card className="shadow-xl border-0">
          <CardHeader className="text-center pb-4">
            <div className="flex justify-center mb-4">
              <div className="w-14 h-14 bg-gradient-to-r from-teal-500 to-purple-500 rounded-2xl flex items-center justify-center shadow-lg">
                <Zap className="w-8 h-8 text-white" />
              </div>
            </div>
            <CardTitle className="text-2xl font-bold">Join Xova</CardTitle>
            <CardDescription>
              Create your account to start generating personalized smoothies
            </CardDescription>
          </CardHeader>
